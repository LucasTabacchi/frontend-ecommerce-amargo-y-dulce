...
export default function GraciasPageClient({
  ...
  const orderNumberRef = useRef(initialOrderNumber);
  const clearedRef = useRef(false);
  const resolvedRef = useRef<StatusKind | null>(
    initialStatus === "success" || initialStatus === "failure" ? initialStatus : null
  );
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        const res = await fetch(`/api/orders/${encodeURIComponent(initialOrderId)}`, {
          cache: "no-store",
        });
  ...
  }, [initialOrderId, clear, hasHydrated]);
        ...
        <div className="py-10">
          ...
          <div className="mt-6 rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              {status === "success" && initialOrderId ? (
                <Link
                  href={`/mis-pedidos/${encodeURIComponent(initialOrderId)}`}
                  className="rounded-full border px-5 py-2.5 text-sm font-semibold text-neutral-900 hover:bg-neutral-50"
                >
                  Ver mi pedido →
                </Link>
